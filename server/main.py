...
ASSISTANT_SYSTEM = (
    ...
    "4. WORKFLOW FORMAT IS RUNNER-DEPENDENT AND NOT INTERCHANGEABLE:\n"
    "   - runner=\"local\": use ComfyUI's API/prompt format (flat dict keyed by "
    "node id string, each entry has class_type + inputs). If your source workflow "
    "is in graph/save format, convert it — you know the node schemas.\n"
    "   - runner=\"cloud\": use ComfyUI's GRAPH/SAVE format (the JSON you get from "
    "'Save (API Format)' toggle OFF in the ComfyUI web UI — top-level `last_node_id`, "
    "`last_link_id`, `nodes[]` array with each entry having numeric `id` + `type` + "
    "`inputs[]` + `outputs[]` + `widgets_values[]`, and `links[]` array at the "
    "root). The cloud runner's manifest patcher (`_find_node` in "
    "`server/modules/_workflow_shared.py`) walks `workflow[\"nodes\"]` looking for "
    "matching `id`s and touches `widgets_values` at the patch's `widget_index` — "
    "if you send API format instead, the patcher errors with `manifest patch: node "
    "N not found in workflow` because there's no `nodes[]` array to walk. When "
    "debugging a cloud workflow failure, DO NOT convert to API format as a fix — "
    "that breaks the patcher. Look at `server/workflows/tripo_p1_i2m_cloud.json` "
    "or `rodin_gen25_i2m_cloud.json` for reference cloud shape.\n"
